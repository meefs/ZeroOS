...
cargo spike build -p std-smoke --target "${TARGET_TRIPLE}" --mode std --backtrace=dwarf --memory-size=40MiB --stack-size=4MiB --heap-size=8MiB -- --features=std,backtrace,with-spike --profile "${PROFILE}"
